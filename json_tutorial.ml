...
(* The function [Yojson.Basic.from_file] takes a couple optional arguments
   ...
   [string -> Yojson.Basic.json].  [from_file fn] reads the contents of the
...
(* Another helpful function is
   [Yojson.Basic.from_string : string -> Yojson.Basic.json], which
...
(* In utop you could now see the tree that was constructed from what
   ...
   ("people",
    `List
      [`Assoc
         [("first name", `String "Ezra"); ("last name", `String "Cornell");
          ("arrived", `Int 1865)];
       `Assoc
         [("first name", `String "Martha"); ("last name", `String "Pollack");
          ("arrived", `Int 2017)]])]

   The [Yojson.Basic.json] type (henceforth just [json] for short) is defined as
   follows:
   ...
   the file is represented as a [json] value.
*)
...
[https://mjambon.github.io/mjambon2016/yojson-doc/Yojson.Basic.Util.html].
The important ones for our purposes are:

   - [to_assoc : json -> (string * json) list].  If [j] is [`Assoc lst], then
     [to_assoc j] is just [lst].
   - [to_list : json -> json list].  If [j] is [`List lst], then
     [to_list j] is just [lst].
   - [to_string : json -> string].  If [j] is [`String s], then
     [to_string j] is just [s].
   - [to_int : json -> int].  If [j] is [`Int i], then
     [to_int j] is just [i].
...
(*
  ...
  [List.assoc] can simplify the code we just wrote:
     ...
     - [member : string -> json -> json].  If [j] is [`Assoc lst], and
